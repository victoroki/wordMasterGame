...
async function init() {
    ...
    function addLetter(letter) {
        ...
        else current = currentGuess.substring(0, currentGuess.length - 1) + letter;
        letters[currentRow * ANSWER_LENGTH + currentGuess.length - 1].innerText = letter;
    }
    // use tries to enter a guess
    async function commit() {
        if (currentGuess.length !== ANSWER_LENGTH) // do nothing
        return;
        ...
        // correct first
        for(let i = 0; i < ANSWER_LENGTH; i++)if (guessParts[i] === wordParts[i]) {
            // mark as correct
        ...
        for(let i1 = 0; i1 < ANSWER_LENGTH; i1++){
            if (guessParts[i1] === wordParts[i1]) ;
            else if (map[guessParts[i1]] && map[guessParts[i1]] > 0) {
                // mark as close
                allRight = false;
                letters[currentRow * ANSWER_LENGTH + i1].classList.add("close");
                map[guessParts[i1]]--;
            } else {
                // wrong
                allRight = false;
                letters[currentRow * ANSWER_LENGTH + i1].classList.add("wrong");
            }
        }
        currentRow++;
        currentGuess = "";
...
function makeMap(array) {
    const obj = {};
    for(let i = 0; i < array.length; i++)if (obj[array[i]]) obj[array[i]]++;
    else obj[array[i]] = 1;
    return obj;
}
init();

//# sourceMappingURL=index.972d9389.js.map
